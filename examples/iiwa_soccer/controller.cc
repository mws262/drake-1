...
void Controller::ConstructJacobians(
...
VectorXd Controller::ComputeTorquesForContactNotDesired(
  ...
  const VectorXd qdot_robot_des = plan_.GetRobotQQdotAndQddot(
      context.get_time()).segment(nq_robot_, nqd_robot_);
  const VectorXd qddot_robot_des = plan_.GetRobotQQdotAndQddot(
      context.get_time()).tail(nqd_robot_);
...
VectorXd Controller::ComputeTorquesForContactDesiredButNoContact(
    const Context<double>& context) const {
  ...
  v0.segment(get_robot_velocity_start_index_in_v(), nqd_robot_).setZero();

  // Transform the velocities to time derivatives of generalized
  // coordinates.
  auto kinematics_cache = robot_and_ball_tree_.doKinematics(q0, v0);
  const VectorXd qdot0 = robot_and_ball_tree_.transformVelocityToQDot(
      kinematics_cache, v0);
  ...
  const auto& robot_link6 = get_robot_link6_from_robot_and_ball_tree();
  ...
  if (indices_A.front() != robot_link6.get_body_index()) {
    std::swap(points_A, points_B);
    std::swap(indices_A, indices_B);
  } else {
    DRAKE_DEMAND(indices_B.front() == robot_link6.get_body_index());
  }
...
// Computes the control torques when contact is desired and the robot and the
// ball are in contact.
VectorXd Controller::ComputeTorquesForContactDesiredAndContacting(
  ...
  DRAKE_DEMAND(nv == nqd_robot_ + nv_ball_);
  const int num_actuators = robot_and_ball_tree_.get_num_actuators();
  DRAKE_DEMAND(num_actuators == nqd_robot_);
...
const VectorXd cf = z.tail(nc);
  ...
  std::cout << "torque: " << z.head(nqd_robot_).transpose() << std::endl;

  // First nqd_robot_ primal variables are the torques.
  return z.head(nqd_robot_);
}

// Gets the vector of contacts.
std::vector<PointPair<double>> Controller::FindContacts(
  ...
  const auto link6_body = &get_robot_link6_from_robot_and_ball_tree();
...
const RigidBody<double>& Controller::get_ball_from_robot_and_ball_tree() const {
...
const RigidBody<double>& Controller::get_robot_from_robot_and_ball_tree()
    const {
...
const RigidBody<double>& Controller::get_world_from_robot_and_ball_tree()
    const {
  return robot_and_ball_tree_.world();
}

/// Gets the link7 body from the robot and ball tree.
const RigidBody<double>& Controller::get_robot_link7_from_robot_and_ball_tree()
    const {
...
const RigidBody<double>& Controller::get_robot_link5_from_robot_and_ball_tree()
const {
...
const RigidBody<double>& Controller::get_robot_link6_from_robot_and_ball_tree()
    const {
...
/// velocities, v.
int Controller::get_robot_velocity_start_index_in_v() const {
...
/// velocities, v.
int Controller::get_ball_velocity_start_index_in_v() const {
...
int Controller::get_robot_position_start_index_in_x() const {
...
int Controller::get_ball_position_start_index_in_q() const {
  return get_ball_from_robot_and_ball_tree().get_position_start_index();
}

VectorXd Controller::get_all_q(const Context<double>& context) const {
...
Eigen::VectorXd Controller::get_all_v(const Context<double>& context) const {
  const VectorXd robot_qd = get_robot_qd(context);
  const VectorXd ball_v = get_ball_v(context);
  VectorXd v(nv_ball_ + nqd_robot_);

  // Sanity check.
  for (int i = 0; i < v.size(); ++i)
    v[i] = std::numeric_limits<double>::quiet_NaN();

  v.segment(get_robot_velocity_start_index_in_v(), nqd_robot_) = robot_qd;
  v.segment(get_ball_velocity_start_index_in_v(), nv_ball_) = ball_v;
  return v;
}


void Controller::DoPublish(
