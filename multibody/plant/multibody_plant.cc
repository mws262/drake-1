...
std::enable_if_t<std::is_same<U, double>::value, VectorX<U>>
  ...
  constraint::ConstraintSolver<T> solver;
  solver.SolveConstraintProblem(data, zeta, dt, &cf);

  // Compute the generalized contact forces.
  VectorX<T> tau_cf;
  solver.ComputeGeneralizedForceFromConstraintForces(data, cf, &tau_cf);
  return tau_cf;
}
...
SoftConstraintProblemData<T> MultibodyPlant<T>::CalcSoftConstraintProblemData(
  ...
  std::vector<RotationMatrix<T>> R_WC_set;
